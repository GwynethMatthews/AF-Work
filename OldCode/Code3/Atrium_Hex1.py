...
class Atrium():
    ...
    """
    pr = cProfile.Profile()
    def __init__(self,L=200,v_para=0.6,v_tran_r= 0.6,v_tran_l= 0.6,d=0.05,e=0.05,rp=50,tot_time = 10**6
                 ,pace_rate = 220,seed1 = 1,seed2=2,seed3=3):
        self.size = L 
        self.first_col = np.arange(0,self.size*self.size,self.size)
        self.transverse_prob_l = v_tran_l
        self.transverse_prob_r = v_tran_r
        ...
        self.index = np.arange(0,self.size*self.size) # cell positions in each array
        ...
        self.phases = np.full((self.size*self.size),fill_value = self.rp) # state cell is in (0 = excited, rp = resting)
        ...
        z = np.random.rand(self.size*self.size)
        y = np.random.rand(self.size*self.size)
