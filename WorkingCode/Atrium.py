...
class Atrium():
    ...
    """
    def __init__(self, hexagonal = False, L = 200, v_para = 1, v_tran_1 = 0.1,
        ...
        if self.hexagonal == False:
            ...
            for j in self.index:
                ...
                #if j in np.arange(500,10500,self.size):
                 #   self.dysfunctional_cells[j] = False
            for j in self.index:
    ...
    def SinusRhythm(self):
        ...
        #self.tbe[100*self.size +100] = True
    ...
    def CMP2D_time_AF(self):
        ...
        while self.t < self.tot_time:
            ...
            # is in SR
            if self.t_AF != 0 and (self.excitations[self.first_col]).max() == (self.excitations).max():
                self.tot_AF += self.t_AF
                self.t_AF = 0
                    
            # is in AF        
            if (self.excitations[self.first_col]).max() < (self.excitations).max():
                self.t_AF += 1
